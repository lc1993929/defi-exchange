    ...
    function addLiquidity(uint _amount) public payable returns (uint) {
        ...
        */
        if (cryptoDevTokenReserve == 0) {
            //  第一次增加代币，将客户的代币转移到该交易合约中。并增加当前eth余额的流动性，
            //            然后根据流动性分配对应的流动性代币给客户。此时的流动性比例等于（当前eth余额：客户提供目标代币金额）
            ...
            */
            ...
            //    增加流动性时，为了保证当前eth和目标代币比例不变，需要根据用户提供的eth余额计算出用户至少需要提供多少代币
            ...
            //            （发送给用户的LP代币（新增的流动性）/合约中LP代币的总供应量）=（用户发送的Eth）/（合约中的Eth储备）
...
*/
    function ethToCryptoDevToken(uint _minTokens) public payable {
        ...
        //保证返回给客户的代币数量不低于客户期望的最低值，否则取消这次交易
